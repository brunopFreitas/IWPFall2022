                    ...
                    <div class="card">
                        <div class="card-body border">
                            ...
                            @foreach($post->users as $user)
                                @isset(Auth::user()->id)
                                    @if(Auth::user()->id == $user->id || $isModerator)
                                        <form class="float-end" method="post" action="{{ route('posts.destroy', $post->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-danger" type="submit">Delete</button> {{--current user or moderator--}}
                                        </form>
                                    @endif
                                    @if(Auth::user()->id == $user->id)
                                        <a href="{{route('posts.edit', $post->id)}}" class="btn btn-primary card-link float-end">Edit</a> {{--only for posts belonging to the user--}}
                                    @endif
                                @endisset
                            @endforeach
                        </div>
